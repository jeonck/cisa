...
| ID | 통제 목적 | 감사 수행 절차 | 필수 증적 파일 |
|----|-----------|--------------|---------------|
| **AUD-01** | 모든 변경 사항이 공식 승인을 거쳐 추적되고 있는가? | ITSM 시스템에서 샘플 기간 내 발행된 변경 요청서(CR)와 승인 이력을 교차 검증 | 변경 승인 이력 로그<br/>긴급 변경 사후 승인서 |
| **AUD-02** | 패치 작업 시 직무 분리(SoD)가 이루어졌는가? | 패치 요청자(보안팀)와 프로덕션 반영자(운영팀)의 계정이 분리되어 있는지 IAM 시스템에서 확인 | IAM 권한 매트릭스 리포트<br/>작업자 접근 로그 |
| **AUD-03** | 패치 실패에 대한 복구 대책이 실효성 있게 수립되었는가? | 무작위 선택 5건의 변경 작업 지시서 내 롤백 시나리오와 사전 백업 수행 기록 포함 여부 검토 | 백업 성공 로그<br/>작업 지시서(롤백 계획 포함) |
| **AUD-04** | 미패치 취약점에 대한 대체 통제가 존재하는가? | SLA 초과 패치 유예 서버 목록을 추출하고, 가상 패치 및 방화벽 차단 정책 활성화 여부 확인 | 예외 처리 승인서<br/>방화벽/IPS 룰셋 적용 스크린샷 |
